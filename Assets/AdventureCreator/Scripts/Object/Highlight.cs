	...
	public class Highlight : MonoBehaviour
	{
		...
		public void _Update ()
		{
			...
			{	
				...
				else if (direction < 0)
				{
					...
					//highlight = highlightCurve.Evaluate (highlightCurve.keys[highlightCurve.length-1].time - timeProportion);
					highlight = highlightCurve.Evaluate(timeProportion);
		...
		private float MaxHighlight
		{
			get
			{
				...
				{ 
					return highlightCurve.keys[highlightCurve.length - 1].value;
				}
				return 2f;
			}
		}
